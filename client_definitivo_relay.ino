...
// ================================================================
//  PARSER UNIVERSALE BEACON
// ================================================================
String parseBeaconPayload(BLEAdvertisedDevice& dev) {
    String result = "";
    char buf[128];
    ...
    if (dev.haveManufacturerData()) {
        ...
        if (mfr.length() >= 3 &&
            (uint8_t)mfr[0] == ELA_CIN_LSB &&
            (uint8_t)mfr[1] == ELA_CIN_MSB) {

            uint8_t dataId = (uint8_t)mfr[2];
            switch (dataId) {
                case ELA_ID_RHT:
                    if (mfr.length() >= 6) {
                        uint8_t rh  = (uint8_t)mfr[3];
                        int16_t raw = (int16_t)(((uint8_t)mfr[5]<<8)|(uint8_t)mfr[4]);
                        snprintf(buf, sizeof(buf), "temp=%.2f;hum=%d;rssi=%d", raw/100.0f, rh, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_TEMP:
                    if (mfr.length() >= 5) {
                        int16_t raw = (int16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        snprintf(buf, sizeof(buf), "temp=%.2f;rssi=%d", raw/100.0f, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_PIR:
                    if (mfr.length() >= 5) {
                        uint16_t raw = (uint16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        snprintf(buf, sizeof(buf), "pir=%s;cnt=%d;rssi=%d",
                            (raw&0x01)?"DETECTED":"CLEAR", raw>>1, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_MOV:
                    if (mfr.length() >= 5) {
                        uint16_t raw = (uint16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        snprintf(buf, sizeof(buf), "mov=%s;cnt=%d;rssi=%d",
                            (raw&0x01)?"IN_PROGRESS":"STOPPED", raw>>1, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_MAG:
                    if (mfr.length() >= 5) {
                        uint16_t raw = (uint16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        snprintf(buf, sizeof(buf), "mag=%s;cnt=%d;rssi=%d",
                            (raw&0x01)?"PRESENT":"ABSENT", raw>>1, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_DI:
                    if (mfr.length() >= 5) {
                        uint16_t raw = (uint16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        snprintf(buf, sizeof(buf), "di=%s;cnt=%d;rssi=%d",
                            (raw&0x01)?"ACTIVE":"RELEASED", raw>>1, rssi);
                        return String(buf);
                    } break;
                case ELA_ID_ANG:
                    if (mfr.length() >= 9) {
                        int16_t x=(int16_t)(((uint8_t)mfr[4]<<8)|(uint8_t)mfr[3]);
                        int16_t y=(int16_t)(((uint8_t)mfr[6]<<8)|(uint8_t)mfr[5]);
                        int16_t z=(int16_t)(((uint8_t)mfr[8]<<8)|(uint8_t)mfr[7]);
                        snprintf(buf, sizeof(buf), "ax=%d;ay=%d;az=%d;rssi=%d", x, y, z, rssi);
                        return String(buf);
                    } break;
                default:
                    snprintf(buf, sizeof(buf), "ela_id=0x%02X;rssi=%d", dataId, rssi);
                    result = String(buf);
                    break;
            }
        } else {
            if (mfr.length() >= 2) {
                uint16_t cid = (uint16_t)(((uint8_t)mfr[1]<<8)|(uint8_t)mfr[0]);
                String raw_hex = "";
                for (int b=0; b<mfr.length()&&b<16; b++) {
                    char h[3]; sprintf(h,"%02X",(uint8_t)mfr[b]); raw_hex += h;
                }
                snprintf(buf, sizeof(buf), "mfr_cid=0x%04X;raw=%s;rssi=%d",
                    cid, raw_hex.c_str(), rssi);
                result = String(buf);
            }
        }
    }
    ...
    if (dev.haveServiceData()) {
        ...
        int16_t pirData=-1, movData=-1;

        for (int s=0; s<dev.getServiceDataCount(); s++) {
            BLEUUID svcUUID = dev.getServiceDataUUID(s);
            String  svcData = dev.getServiceData(s);

            if (svcUUID.equals(BLEUUID(BT_UUID_TEMPERATURE)) && svcData.length()>=2) {
                temperature = (int16_t)(((uint8_t)svcData[1]<<8)|(uint8_t)svcData[0]) / 100.0f;
            } else if (svcUUID.equals(BLEUUID(BT_UUID_HUMIDITY)) && svcData.length()>=1) {
                humidity = (uint8_t)svcData[0];
            } else if (svcUUID.equals(BLEUUID(BT_UUID_RAINFALL)) && svcData.length()>=2) {
                pirData = (int16_t)(((uint8_t)svcData[1]<<8)|(uint8_t)svcData[0]);
            } else if (svcUUID.equals(BLEUUID(BT_UUID_ALERT_LEVEL)) && svcData.length()>=2) {
                movData = (int16_t)(((uint8_t)svcData[1]<<8)|(uint8_t)svcData[0]);
            } else if (svcUUID.equals(BLEUUID(BT_UUID_MAG3D)) && svcData.length()>=6) {
                int16_t x=(int16_t)(((uint8_t)svcData[1]<<8)|(uint8_t)svcData[0]);
                int16_t y=(int16_t)(((uint8_t)svcData[3]<<8)|(uint8_t)svcData[2]);
                int16_t z=(int16_t)(((uint8_t)svcData[5]<<8)|(uint8_t)svcData[4]);
                snprintf(buf, sizeof(buf), "ax=%d;ay=%d;az=%d;rssi=%d", x, y, z, rssi);
                return String(buf);
            } else {
                String sd_hex="";
                for(int b=0;b<svcData.length()&&b<8;b++){
                    char h[3]; sprintf(h,"%02X",(uint8_t)svcData[b]); sd_hex+=h;
                }
                char field[80];
                snprintf(field,sizeof(field),"sd_%s=%s",svcUUID.toString().c_str(),sd_hex.c_str());
        ...
        String svcResult="";
        if (!isnan(temperature)) { snprintf(buf,sizeof(buf),"temp=%.2f",temperature); svcResult+=buf; }
        if (!isnan(humidity))    { if(svcResult.length())svcResult+=";"; snprintf(buf,sizeof(buf),"hum=%.0f",humidity); svcResult+=buf; }
        if (pirData>=0)          { if(svcResult.length())svcResult+=";"; snprintf(buf,sizeof(buf),"pir=%s;cnt=%d",(pirData&0x01)?"DETECTED":"CLEAR",pirData>>1); svcResult+=buf; }
        if (movData>=0)          { if(svcResult.length())svcResult+=";"; snprintf(buf,sizeof(buf),"mov=%s;cnt=%d",(movData&0x01)?"IN_PROGRESS":"STOPPED",movData>>1); svcResult+=buf; }
        if (svcResult.length()>0) result = (result.length()>0) ? svcResult+";"+result : svcResult;
    }

    if (result.length()>0 && result.indexOf("rssi=")<0) {
        snprintf(buf,sizeof(buf),";rssi=%d",rssi); result+=buf;
    }
    return result;
}
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        for (int i=0;i<activeSensors;i++) {
            ...
            String dataStr=parseBeaconPayload(advertisedDevice);
            if (dataStr.length()>0) {
                if (xSemaphoreTake(dataMutex,(TickType_t)10)==pdTRUE) {
                    sensorDataBuffer[i]=dataStr; sensorDataReady[i]=true;
                    xSemaphoreGive(dataMutex);
                }
                updateHubCharStatus(i,"Connected, "+dataStr);
                connected[i]=true; lastSeenTime[i]=millis();
                safeSerialPrintf("[BEACON] Slot %d | MAC:%s | %s\n",i+1,foundMac.c_str(),dataStr.c_str());
            } else {
                safeSerialPrintf("[BEACON] Slot %d payload non decodificato (haveSD:%d haveMfr:%d RSSI:%d)\n",
                    i+1,advertisedDevice.haveServiceData()?1:0,
                    advertisedDevice.haveManufacturerData()?1:0,advertisedDevice.getRSSI());
                if (advertisedDevice.haveManufacturerData()) {
                    String mfr=advertisedDevice.getManufacturerData();
                    safeSerialPrintf("[DUMP MFR] len:%d:",mfr.length());
                    for(int b=0;b<mfr.length();b++) safeSerialPrintf(" %02X",(uint8_t)mfr[b]);
                    safeSerialPrintf("\n");
                }
            }
            return;
...
void bleTask(void* parameter) {
    ...
    for (;;) {
        if (currentMode==1) { vTaskDelay(100/portTICK_PERIOD_MS); continue; }
...
//  - Keepalive 300s: evita disconnessioni per inattivita'
// ================================================================
    ...
    for (;;) {
        ...
        if (currentMode != 1 && mqttConnected) {
            ...
            for (int i = 0; i < activeSensors; i++) {
                ...
                if (nd == "") continue;
                String msg = String(hubName) + ":" + String(i+1) + ":" + nd;
                String cmd = "AT+MPUB=\"modem/display\",0,0,\"" + msg + "\"";
                Serial1.println(cmd);
                safeSerialPrintln("TX MQTT: " + cmd);
                vTaskDelay(150 / portTICK_PERIOD_MS);  // il modem digerisce prima del prossimo
            }
            // ─────────────────────────────────────────────────────
        }
...
void setup() {
    ...
    if (currentMode==1) {
        ...
        for(int i=0;i<activeSensors;i++){
            char uM[40],uS[40],uC[40];
            const char* fmt=(i<9)?"00000000-0000-0000-0000-0000000000%d%d":"00000000-0000-0000-0000-000000000%d%d";
            sprintf(uM,fmt,i+1,2); sprintf(uS,fmt,i+1,0); sprintf(uC,fmt,i+1,1);
            mkChar(uM,new ConfigCallbacks("mac_"   +String(i)),preferences.getString(("mac_"   +String(i)).c_str(),""));
            mkChar(uS,new ConfigCallbacks("s_uuid_"+String(i)),preferences.getString(("s_uuid_"+String(i)).c_str(),""));
            mkChar(uC,new ConfigCallbacks("c_uuid_"+String(i)),preferences.getString(("c_uuid_"+String(i)).c_str(),""));
        }
        preferences.end();
        pService->start();
